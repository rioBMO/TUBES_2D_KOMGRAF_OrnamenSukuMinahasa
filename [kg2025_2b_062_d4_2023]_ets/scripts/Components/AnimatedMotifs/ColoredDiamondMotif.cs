...
{
    public class ColoredDiamondMotif : ColoredMotifBase
    {
        private GodotVector2 position;
        private float size;
        private float diamondBreathingFactor = 1.0f;

        public ColoredDiamondMotif(Node2D parent, KartesiusSystem kartesiusSystem,
        ...
        // Override the SetBreathingFactor to update the diamond-specific factor
        public override void SetBreathingFactor(float factor)
        {
            base.SetBreathingFactor(factor);
            this.diamondBreathingFactor = factor;
        }
        ...
        public override void Draw()
        {
            ...
            // Draw filled diamonds from the outside in
            DrawFilledPolygon(outerDiamond, colorPalette.DarkGreen);
            DrawFilledPolygon(middleDiamond, colorPalette.MediumGreen);
            DrawFilledPolygon(innerDiamond, colorPalette.LightOrange);

            // Draw outlines for better definition
            parent.DrawPolyline(outerDiamond, lineColor);
            parent.DrawPolyline(middleDiamond, lineColor);
            parent.DrawPolyline(innerDiamond, lineColor);
        }
    }
}
